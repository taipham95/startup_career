  ...
  return (
    <div className="relative grow font-light text-xs md:text-sm">
      <input
        ...
        required={required}
        autoComplete="off"
        className= {`peer w-full min-h-[auto] text-xs md:text-sm rounded-lg border bg-white py-4 px-3 leading-[1.6] placeholder-gray-500 focus:placeholder-transparent placeholder:text-xs md:placeholder:text-sm outline-none transition-all duration-200 ease-linear motion-reduce:transition-none focus:outline-none focus:shadow-none focus:ring-transparent ${required ? 'dark:focus:border-rose-500 focus:border-rose-500' : 'dark:focus:border-sky-300 focus:border-sky-300'}`}
        value={value || ""}
        placeholder={text}
